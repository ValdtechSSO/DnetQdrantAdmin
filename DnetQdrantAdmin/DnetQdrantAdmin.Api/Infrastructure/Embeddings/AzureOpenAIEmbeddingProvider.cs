...
public class AzureOpenAIEmbeddingProvider : IEmbeddingProvider
{
    ...
    public AzureOpenAIEmbeddingProvider(EmbeddingProviderConfig config, IServiceProvider serviceProvider)
    {
        _config = config;
    ...
    public async Task<IReadOnlyList<ReadOnlyMemory<float>>> GenerateAsync(IEnumerable<string> inputs, ModelConfig model, int dimension, CancellationToken cancellationToken = default)
    {
        ...
        var generator = _serviceProvider.GetKeyedService<IEmbeddingGenerator<string, Embedding<float>>>(serviceId)
            ?? throw new InvalidOperationException($"No embedding generator is registered for provider '{Name}', deployment '{model.Model}' with {dimension} dimensions");
